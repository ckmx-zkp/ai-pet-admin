...
const persona = reactive({ sun_sign: '', mbti: '', overridesText: '{}', follow_latest: true, identity: '', background: '', roles: '', goals: '', evolutionRules: '', relationship: '' })
...
async function loadPersona() {
  ...
  try {
    ...
    persona.overridesText = pretty(result.overrides); persona.follow_latest = result.follow_latest; persona.identity = result.dossier?.identity || ''; persona.background = (result.dossier?.background || []).join('\n'); persona.roles = (result.dossier?.roles || []).join('\n'); persona.goals = (result.dossier?.goals || []).join('\n'); persona.evolutionRules = (result.dossier?.evolution_rules || []).join('\n'); persona.relationship = result.dossier?.relationship || ''; personaExists.value = true
  } catch (error: any) {
...
async function savePersona() {
  ...
  try {
    ...
    await updateAdminPersona(props.id, { sun_sign: persona.sun_sign, mbti: persona.mbti, overrides, follow_latest: persona.follow_latest, dossier: { identity: persona.identity.trim(), background: list(persona.background), roles: list(persona.roles), goals: list(persona.goals), evolution_rules: list(persona.evolutionRules), relationship: persona.relationship.trim() } })
...
async function loadMessages(offset = messageOffset.value) {
  ...
  } catch (error: any) { detailError(error, '脱敏历史加载失败') } finally { tabLoading.messages = false }
}
...
async function loadAnalyses(offset = analysisOffset.value) { tabLoading.analyses = true; try { const result = await listAdminAnalyses(props.id, { kind: analysisKind.value || undefined, limit: messagePageSize, offset }); analyses.value = result.data; analysisOffset.value = offset; hasMoreAnalyses.value = result.data.length === messagePageSize } catch (error: any) { detailError(error, '分析记录加载失败') } finally { tabLoading.analyses = false } }
  ...
  <section>
    ...
    <template v-else>
      ...
      <el-card class="detail-card" shadow="never"><el-tabs v-model="activeTab" @tab-change="loadTab">
        <el-tab-pane label="人设" name="persona"><el-alert v-if="!canEditPersona" title="设备尚未由用户认领，不能配置人设。" type="warning" :closable="false" show-icon class="tab-notice" /><el-form v-loading="tabLoading.persona" label-width="90px" class="persona-form"><el-form-item label="星座"><el-select v-model="persona.sun_sign" :disabled="!canEditPersona" placeholder="选择星座"><el-option v-for="item in signs" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item><el-form-item label="MBTI"><el-select v-model="persona.mbti" :disabled="!canEditPersona" placeholder="选择 MBTI"><el-option v-for="item in mbtis" :key="item" :label="item" :value="item" /></el-select></el-form-item><el-form-item label="角色身份"><el-input v-model="persona.identity" :disabled="!canEditPersona" placeholder="例如：温柔的陪伴型 AI 宠物" /></el-form-item><el-form-item label="背景/角色/目标"><el-input v-model="persona.background" type="textarea" :rows="2" placeholder="每行一项背景" /><el-input v-model="persona.roles" type="textarea" :rows="2" placeholder="每行一项角色" /><el-input v-model="persona.goals" type="textarea" :rows="2" placeholder="每行一个目标" /></el-form-item><el-form-item label="关系与进化"><el-input v-model="persona.relationship" placeholder="与主人的关系" /><el-input v-model="persona.evolutionRules" type="textarea" :rows="2" placeholder="每行一条进化规则" /></el-form-item><el-form-item label="跟随最新"><el-switch v-model="persona.follow_latest" :disabled="!canEditPersona" /></el-form-item><el-form-item label="高级配置"><el-input v-model="persona.overridesText" :disabled="!canEditPersona" type="textarea" :rows="3" /></el-form-item><el-form-item><el-button type="primary" :disabled="!canEditPersona" :loading="tabLoading.persona" @click="savePersona">{{ personaExists ? '保存人设' : '创建人设' }}</el-button></el-form-item></el-form></el-tab-pane>
        ...
        <el-tab-pane label="分析" name="analyses"><div class="history-tools"><el-input v-model="analysisKind" placeholder="按类型筛选，例如 daily_summary" clearable @keyup.enter="loadAnalyses(0)" /><el-button type="primary" :loading="tabLoading.analyses" @click="loadAnalyses(0)">筛选</el-button></div><el-table v-loading="tabLoading.analyses" :data="analyses" empty-text="暂无分析记录"><el-table-column prop="kind" label="类型" width="160" /><el-table-column label="生成时间" width="180"><template #default="{ row }">{{ formatTime(row.created_at) }}</template></el-table-column><el-table-column label="结果"><template #default="{ row }"><pre>{{ pretty(row.payload) }}</pre></template></el-table-column></el-table><div class="pagination"><el-button :disabled="analysisOffset === 0 || tabLoading.analyses" @click="loadAnalyses(Math.max(0, analysisOffset - messagePageSize))">上一页</el-button><span>第 {{ Math.floor(analysisOffset / messagePageSize) + 1 }} 页</span><el-button :disabled="!hasMoreAnalyses || tabLoading.analyses" @click="loadAnalyses(analysisOffset + messagePageSize)">下一页</el-button></div></el-tab-pane>
      </el-tabs></el-card>
    </template>
  </section>
</template>
...
<style scoped>
.detail-card { max-width: 920px; margin-top: 24px; }.card-header { display: flex; justify-content: space-between; gap: 16px; }.card-header p { margin: 6px 0 0; color: #6b7280; }.status { margin-left: 8px; }.binding-id { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; word-break: break-all; }.capability { margin: 2px 6px 2px 0; }.actions { margin-top: 20px; }.tab-notice { margin-bottom: 20px; }.persona-form { max-width: 640px; }.history-tools, .pagination { display: flex; align-items: center; gap: 12px; margin-bottom: 16px; }.pagination { justify-content: flex-end; margin: 16px 0 0; } pre { margin: 0; white-space: pre-wrap; word-break: break-word; }
@media (max-width: 680px) { .history-tools { align-items: stretch; flex-direction: column; }.history-tools :deep(.el-date-editor) { width: 100%; }.pagination { justify-content: space-between; } }
</style>
